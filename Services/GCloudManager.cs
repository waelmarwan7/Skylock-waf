...
public class GCloudManager
{
    ...
    public async Task SetupInfrastructure(string networkName, string userName , string ip_address , int id)
    {
        try
        {
            ...
            await CreateInstanceGroupAsync(imageName,ip_address);
    ...
    private async Task CreateInstanceGroupAsync(string imageName, string ipAddress)
    {
        ...
        string templateCommand = $"compute instance-templates create {instanceGroupName}-template " +
                            ...
                            $"--network={networkName} " +
                            $"--subnet=projects/{projectId}/regions/{region}/subnetworks/{subnetName} " + // Full subnet path
                            $"--image={imageName} " +
                            $"--metadata-from-file=startup-script={tempScriptPath} " +
                            $"--boot-disk-type=pd-ssd " +
                            // $"--metadata {sshKeyMetadata} " + // Added SSH key metadata
                            $"--region={region} " ;
        ...
        // Create managed instance group - FIXED COMMAND
        string migCommand = $"compute instance-groups managed create {instanceGroupName} " +
                $"--project={projectId} " +
                $"--template={templateName} " +  // Added space after template name
                $"--size=1 " +                  // Properly formatted size parameter
        ...
        // Set named ports (using region-based command)
    ...
    {
        // Add retry logic for IP retrieval
        int retries = 5;
